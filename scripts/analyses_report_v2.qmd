...
missing_rids <- data %>% filter(is.na(Match)) %>% select(ResearchID)
...
# *** Need to figure out what to do with the duplicate match data
# *** What to do with the folks with NA for match
# *** We only have 8 FM placements out 155 total.
```
...
```{r}
# Dependent variable is whether or not they matched in FM or not
